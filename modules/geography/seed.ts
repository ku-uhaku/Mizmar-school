 ...
 *
 * `School.city` is free text — a school types where it is — so the seed has to
 * meet it on the name rather than on a code it never had. Undefined for a town
 * outside the starting list, which leaves the quartier seed unfiltered rather
...
/**
 * Quartiers for each town a school in this seed might stand in, so the bus stops
 * the transport seed lays down have somewhere to be.
 *
 * The whole list is never given to one school — `seedNeighbourhoods` takes the
 * town and lays down only its own. Keyed on the city *code* rather than an id,
 * so the caller passes the map `seedCities` just returned and the two stay in
 * step.
 */
