...
/**
 * Maps permission codes to ids, silently dropping anything not in the catalogue.
 * A role can therefore never hold a permission the code does not check.
 */
async function resolvePermissionIds(codes: string[]): Promise<string[]> {
  const allowed = codes.filter((code) =>
    (ALL_PERMISSION_CODES as string[]).includes(code),
  );
