...
import { authorizeOrg, authorizeSchool, requireAuth } from "@/lib/dal";
import { db } from "@/lib/db";
import { getDictionary } from "@/lib/i18n/server";
import { PERMISSIONS } from "@/lib/permissions";
import { boolField, field, withActionErrors } from "@/lib/server-action";
import { fieldErrors } from "@/lib/validation/common";
import { schoolSchema } from "@/lib/validation/schemas";
...
/** Used by the schools table to jump straight into a school's context. */
export async function setCurrentSchoolAction(
  schoolId: string,
): Promise<ActionState> {
  return withActionErrors(async () => {
    const t = await getDictionary();
    ...
    if (!context.schools.some((school) => school.id === schoolId)) {
    ...
    await db.user.update({
      ...
      data: { currentSchoolId: schoolId, currentSchoolYearId: nextYear?.id ?? null },
    });
